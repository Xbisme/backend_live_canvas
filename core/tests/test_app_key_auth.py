...
Uses the temporary ``/_probe/app-tier`` view. Asserts the gate rejects missing/wrong
keys with the ``INVALID_APP_KEY`` envelope, is isolated from the admin Bearer tier,
fails closed on an empty configured key, leaves health endpoints ungated, and never
logs the key value.
"""
...
def test_missing_key_rejected(client: APIClient) -> None:
    _assert_invalid_app_key(client.get("/_probe/app-tier"))


@override_settings(X_APP_KEY=_KEY)
def test_wrong_key_rejected(client: APIClient) -> None:
    _assert_invalid_app_key(client.get("/_probe/app-tier", HTTP_X_APP_KEY="nope"))
...
def test_bearer_only_rejected_no_cross_tier_fallback(client: APIClient) -> None:
    ...
    response = client.get("/_probe/app-tier", HTTP_AUTHORIZATION="Bearer some.jwt.token")
    _assert_invalid_app_key(response)


@override_settings(X_APP_KEY=_KEY)
def test_correct_key_accepted(client: APIClient) -> None:
    response = client.get("/_probe/app-tier", HTTP_X_APP_KEY=_KEY)
...
def test_empty_configured_key_denies_everyone(client: APIClient) -> None:
    ...
    _assert_invalid_app_key(client.get("/_probe/app-tier", HTTP_X_APP_KEY=""))
    _assert_invalid_app_key(client.get("/_probe/app-tier", HTTP_X_APP_KEY="anything"))
...
def test_key_value_never_logged(client: APIClient, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        client.get("/_probe/app-tier", HTTP_X_APP_KEY=wrong)
